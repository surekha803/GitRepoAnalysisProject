...
distinct_repoid = df2['repo.id'].unique()
repoids = distinct_repoid.tolist()
print("$$$$$$$$$$$$", repoids)

for ids in repoids:
    query_job = bqclient.query("""
                  select  distinct actor.id as actor_ids from """+"`"+"github-project-310921.githubproject_rawdataset.20*"""+"`"+""" where  type='PullRequestEvent' and repo.id="'"+ids+"'" """).result().to_dataframe(bqstorage_client=bqstorageclient)
    actorids=query_job.stack().tolist()
# CC_forallrepos = repoidslist.loc[repoidslist['actorid'].isin(actorids)]
